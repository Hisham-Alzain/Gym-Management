...
class ExerciseController extends GetxController {
  ...
  Future<void> onInit() async {
    ...
    //   Uri(
    //     path: 'http://192.168.0.105:8000/api/video/${exercise.videoPath}',
    //   ),
    // )..initialize().then((_) {
    //     update();
    //   });
  ...
  Future<dynamic> getExercise(int id) async {
    ...
    try {
      var response = await dio.get(
        'http://192.168.0.105:8000/api/trainee/exercise/$id',
        options: Options(
          headers: {
  ...
  Future<dynamic> addSet(
    ...
    try {
      var response = await dio.post(
        'http://192.168.0.105:8000/api/trainee/workout/set',
        options: Options(
          headers: {
            'Content-Type': 'application/json; charset=UTF-8',
            'Accept': 'application/json',
